...
async def send_telegram_message(bot_token, chat_id, message):
    ...
    This function will raise a TelegramError on failure.
    """
    if not message:
    ...
    try:
        ...
        # Log the critical error and then re-raise it to be caught by the main loop.
...
async def main():
    logging.info("--- Summarizer Bot v4.4 (Transactional) Run Started ---")
    ...
    if structured_summary.startswith("ERROR:"):
        # Send a simple error message but DO NOT clear the log
        await send_telegram_message(telegram_token, chat_id, sanitize_markdown_v2(structured_summary))
        logging.error("Aborting run due to Gemini API error. Daily log will be preserved for next run.")
        return
    ...
    if not message_parts:
         logging.warning("AI response was empty or unparsable. Daily log will be preserved for next run.")
         return
    ...
    # --- Step 5: Send Message and Clean Up on Success ---
    try:
        await send_telegram_message(telegram_token, chat_id, final_message)
        # THIS LINE ONLY RUNS IF THE ABOVE LINE DOES NOT RAISE AN ERROR
        clear_daily_log(log_file)
    except TelegramError:
        # If sending fails, the error is already logged by send_telegram_message.
        # We explicitly DO NOT clear the log, so it can be retried on the next run.
        logging.error("Telegram sending failed. Preserving daily log for the next attempt.")
    
    logging.info("--- Summarizer Bot v4.4 Run Finished ---")
